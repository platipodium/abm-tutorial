...
butterflies-own [
  counter ; count how many times a butterfly hits the wall
]

; Define the environment and how the simulation starts
to setup

  clear-all             ; create a blank screen
  setup-patches
  setup-butterflies

end

; Define an infinite loop that tells all turtles to move around
to go
  ask turtles [
    move-turtle
    if counter > 10 [change-turtle]
  ]
end
...
; Define how the moving agents "turtles" start
to setup-butterflies
  create-butterflies 8 [
...
; This is a turtle procedure, i.e. the message "move-turtles" must be directed to a turtle
...
to move-turtle
  ...
  ifelse target-patch != nobody [
    ifelse (  ([pxcor] of target-patch )  * xcor > 0 ; same left-right of butterfly and target patch
      and ([pycor] of target-patch )  * ycor > 0 ; same top-down of butterfly and target patch
    ) [forward step-size][set counter counter + 1]
  ][
...
to change-turtle
  ...
  ; Choose one shape different from the current shape
  set shape one-of remove shape (list "cow" "butterfly" "bug" "fish" "sheep" "person" )
end
@#$#@#$#@
GRAPHICS-WINDOW
238
...
161
38
Click \"setup\" first, then \"go\" and observe what happens
11
0.0
1
...
164
154
Adjust the \"speed\" of the butterflies and adjust their size.  Both work differently!
11
0.0
1
